...
| Megolm key backup (originate + honesty) | Done (ADR 0098) — `AccountDto.backup` snapshot, recover auto-enable/join/export-resume, `POST /v1/accounts/{id}/backup/enable`, recover flatten + `redecrypt`/`backup_action` | Done — `/backup enable [user]` (masked key; empty Enter kicks upload) and `/status` snapshot; `/recover` unchanged | **Planned** — still says "Keys recovered"; badge + enable control are a follow-up | Not started | Server GET cache of `exists_on_server` may lag another client; enable then 409s. No `account.backup` WS in this PR. Recover does not decrypt pre-Axon history |
