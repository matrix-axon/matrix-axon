...
| Megolm key backup (originate + honesty) | Done (ADR 0098) — `AccountDto.backup` snapshot, recover auto-enable/join/export-resume, `POST /v1/accounts/{id}/backup/enable`, recover flatten + `redecrypt`/`backup_action` | Done — `/backup enable [user]` (masked key; empty Enter kicks upload) and `/status` snapshot; `/recover` unchanged | Done — account-card snapshot + badge, honest recover/login notices (`backup_action`, never "Keys recovered"), Enable backup (optional key; empty submits kick-upload) | Not started | Server GET cache of `exists_on_server` may lag another client; enable then 409s. No `account.backup` WS in this PR. Recover does not decrypt pre-Axon history |
