...
| Invited-room visibility (see incoming invites, accept/reject) | Done (ADR 0091) — persisted `room_invites`, `GET /v1/invites`, `invite.added` / `invite.removed` WS frames. Accept/reject reuse existing join/leave. | Not started | Done — Invites row + `/invites` inbox with Accept/Reject and Accept all/Reject all | Not started | TUI stays without an inbox; known-contact DM auto-join (ADR 0040) is unchanged |
