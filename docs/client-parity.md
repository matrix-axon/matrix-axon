...
| Interactive SAS device verification | Done (7a-6, ADR 0027/0028) | Done — full emoji-modal flow | Done — Accounts picker + emoji modal; inbound attention band above Invites and a topbar chip; Esc parks | Not started | Web uses `GET …/devices` rather than TUI's pasted id; no auto-open |
...
| Device-list / picker endpoint | Done (M16, ADR 0060) | **Gap** — no picker UI; verification still requires a blind device id | Done — `DevicePicker` on Accounts consumes `GET …/devices` (self only) | Not started | TUI still pastes a device id |
