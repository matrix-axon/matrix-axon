...
async fn verification_provider() -> (AppleProvider, tokio::task::JoinHandle<()>) {
    let (base, task) = serve(Router::new().route("/keys", get(|| async {
        Json(json!({"keys": [{"kty":"RSA", "kid":"apple-test", "alg":"RS256", "use":"sig", "n":RSA_N, "e":"AQAB"}]}))
    }))).await;
...
fn sign(claims: &Value) -> String {
    ...
    encode(
        &header,
        claims,
        &EncodingKey::from_rsa_pem(RSA_KEY.as_bytes()).unwrap(),
    )
    .unwrap()
}
...
fn verify_secret(secret: &str) -> Value {
    ...
    decode::<Value>(
        secret,
        &DecodingKey::from_ec_components(TEST_EC_X, TEST_EC_Y).unwrap(),
        &validation,
    )
    .unwrap()
    .claims
