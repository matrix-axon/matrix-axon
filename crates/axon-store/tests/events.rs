...
/// `event_positions` reports both orderings independently, so a caller can tell
/// "arrived later" from "displays later". The shape here is the one a mautrix
/// portal produces: the bridge creates the room, then backfills the
/// pre-existing conversation with its *original* `origin_server_ts`, so the
/// message is the newest row by ingest order (`id`) and the oldest by
/// `origin_ts`. Unknown ids are absent rather than an error, and the lookup is
/// scoped to the requested room.
#[tokio::test]
#[ignore = "requires Postgres"]
async fn event_positions_reports_arrival_order_independently_of_origin_ts() {
    let store = common::migrated_store().await;
    let pool = common::raw_pool().await;
    let account_id = common::test_account(&store, "event-positions").await;
    let room_id = format!("!r-{}:localhost", Uuid::new_v4());
    let other_room_id = format!("!other-{}:localhost", Uuid::new_v4());

    // Inserted first, and last by `origin_ts` — the portal's own state churn.
    let portal_state = insert_message(&store, account_id, &room_id, 309_453, "portal state").await;
    // Inserted second, but stamped 4.5s earlier — the backfilled conversation.
    let backfilled = insert_message(&store, account_id, &room_id, 304_987, "backfilled").await;
    let elsewhere = insert_message(&store, account_id, &other_room_id, 500_000, "elsewhere").await;
    let unknown = format!("$unknown-{}:localhost", Uuid::new_v4());
    ...
    let positions = store
        ...
        .await
        .expect("event_positions");

    assert_eq!(positions.len(), 2, "only this room's known events");
    assert!(!positions.contains_key(&elsewhere), "other room excluded");
    assert!(!positions.contains_key(&unknown), "unknown id absent");

    let state = positions[&portal_state];
    let message = positions[&backfilled];
    assert!(
        message.id > state.id,
        "the backfilled message arrived later ({} vs {})",
        message.id,
        state.id
    );
    assert!(
        message.origin_ts < state.origin_ts,
        "…but displays earlier ({} vs {})",
        message.origin_ts,
        state.origin_ts
    );

    // An empty request never reaches the database.
    let empty = store
        .event_positions(account_id, &room_id, &[])
        .await
        .expect("event_positions with no ids");
